"""
email-open-tracker v2 — FastAPI async pixel server with a live dashboard.
...
Routes:
  GET /pixel.gif?id=<tracking_id>  -> logs an open, returns a 1x1 transparent gif
  GET /                            -> live dashboard (auto-refreshes via /api/opens)
  GET /api/opens                   -> JSON of recent opens (dashboard polls this)
...
<style>
 ...
 .muted{color:#888;font-size:.85rem} .dot{color:#16a34a}
</style>
<h1>Email opens <span class="dot">&#9679;</span> <span id="count" class="muted"></span></h1>
<p class="muted">Live &mdash; refreshes every 5s. Note: Apple Mail preloads images (instant open from
an Apple datacenter) and Gmail proxies images through Google, so IP/location may reflect the
provider, not the recipient.</p>
<table><thead><tr><th>When (UTC)</th><th>ID</th><th>Location</th><th>IP</th><th>Client</th></tr></thead>
<tbody id="rows"></tbody></table>
<script>
async function load(){
  const r = await fetch('/api/opens'); const d = await r.json();
